...
### Error: `KeyError: 'ContainerConfig'` saat `docker-compose up`

Ini **bukan** salah isi `docker-compose.yml`, melainkan bug **kompatibilitas** antara **docker-compose v1** (Python, perintah `docker-compose` dengan tanda hubung) dan Docker Engine versi baru.

**Solusi (pilih salah satu):**

1. **Pakai Docker Compose V2 (disarankan)** — perintah pakai **spasi**, bukan tanda hubung:
   ```bash
   docker compose down
   docker compose up -d
   ```
   Compose V2 biasanya sudah terpasang sebagai plugin Docker (`docker compose version`). Jika belum: [Install Docker Compose Plugin](https://docs.docker.com/compose/install/linux/).

2. **Jika tetap pakai `docker-compose` (v1):** Hapus container yang bermasalah lalu up lagi, agar Compose tidak baca konfigurasi container lama:
   ```bash
   docker-compose down
   docker rm -f $(docker ps -aq -f name=ingestor) 2>/dev/null || true
   docker-compose up -d
   ```

### Dashboard Grafana kosong / metrics tidak tampil

Ikuti **step-by-step** di **[MONITORING_TROUBLESHOOTING.md](./MONITORING_TROUBLESHOOTING.md)**:

1. **Step 1 — Exporter:** Pastikan metric bisa diakses (curl Redis exporter dengan `?target=...`, curl HDFS exporter `/metrics`).
2. **Step 2 — Prometheus:** Cek Status → Targets (semua UP), lalu cek query `redis_up` dan `namenode_*` di Graph/Explore.
3. **Step 3 — Grafana:** Cek datasource Prometheus "Save & test", lalu dashboard Redis/HDFS (variable Instance = All, time range cukup lebar).

### "COPY failed: go.sum" saat build

- Dockerfile sudah diatur agar tidak wajib `go.sum`; `go mod tidy` dijalankan di dalam build. Jika masih error, dari folder `app` jalankan: `go mod tidy` lalu build lagi.

### Redis cluster down / CLUSTERDOWN

- Cek: `docker compose logs redis-cluster-init` — harus ada pesan sukses "create" cluster.
- Restart: `docker compose restart redis-1 redis-2 redis-3` lalu tunggu, atau full ulang: `docker compose down -v` dan `docker compose up -d` (volume akan kosong).

### Ingestor tidak bisa connect Redis / HDFS

- Pastikan dependency: `depends_on: redis-cluster-init, namenode, datanode, datanode-2, datanode-3`.
- Cek log: `docker compose logs ingestor`.
- Pastikan `REDIS_STARTUP_NODES` dan `HDFS_PATH` benar.

### Generator atau Ingestor berhenti setelah beberapa saat

- Keduanya **tidak punya batas waktu** di kode (loop/HTTP server jalan terus). Jika container berhenti, cek: `docker compose ps` (kolom **State** dan **Restart**), lalu `docker compose logs generator` dan `docker compose logs ingestor` untuk penyebab crash.
- Sudah diset **`restart: unless-stopped`** di `docker-compose.yml` untuk ingestor, generator, hotkey-manager, dan offloader — container akan di-restart otomatis. Jika tetap mati berulang, periksa log (mis. connection refused ke Redis/HDFS, OOM).

### Offloader: docker logs kosong (tidak muncul apa-apa)

- Pastikan container yang dicek benar-benar **offloader**: `docker ps -a` lalu cek nama/IMAGE. Atau: `docker compose logs offloader` (dari folder project yang punya docker-compose).
- **Rebuild** image offloader lalu up lagi: `docker compose build offloader && docker compose up -d offloader`. Setelah perbaikan kode, log sekarang muncul di awal: `offloader: process started`, lalu `offloader: connecting to Redis...`, lalu `offloader started: ...`.
- Jika setelah rebuild log tetap kosong: cek `docker inspect <container_id>` → **State** (ExitCode, Error, OOMKilled) dan **Config.Cmd/Entrypoint**. Pastikan entrypoint adalah `/app/offloader`.

### Offloader jalan tapi tidak ada data di HDFS / log "moved=0" terus

- **Redis eviction:** Redis pakai `maxmemory 50mb` dan `allkeys-lru`. Dengan traffic tinggi (RPS 200), key sering **di-evict** sebelum umurnya mencapai `OFFLOAD_AFTER_SECONDS`. Jadi offloader jarang menemukan key yang "sudah lama". **Solusi:** turunkan `OFFLOAD_AFTER_SECONDS`, **turunkan** `OFFLOAD_INTERVAL_SECONDS` (lebih sering scan), aktifkan mode agresif `OFFLOAD_FORCE_MEM_RATIO`, atau naikkan `maxmemory` di Redis untuk simulasi.
- **Log diagnostik:** Setiap run offloader sekarang log: `scanned=... old=... moved=... write_fail=...`. Jika `old > 0` tapi `moved=0` dan `write_fail > 0` → tulis ke HDFS gagal (cek `offload write failed key=...` di log; biasanya JAVA_HOME atau koneksi ke namenode). Jika `old=0` terus → tidak ada key yang berumur > threshold (eviction atau threshold terlalu besar).
- Pastikan image offloader/ingestor **sudah di-rebuild** setelah penambahan `JAVA_HOME` di Dockerfile, agar perintah `hdfs dfs` tidak error "JAVA_HOME is not set".
- Cek log: `docker compose logs offloader` — harus ada baris `offload run: scanned=...` setiap interval; jika ada `write_fail` akan ada baris `offload write failed key=...`.

### HDFS exporter / dashboard HDFS tidak ada data

- Cek target Prometheus untuk job `hdfs` (hdfs-exporter:9070) status UP.
- Cek: `curl http://localhost:9070/metrics` — harus ada metric `namenode_*`.
- Pastikan Namenode sudah sehat: http://localhost:9870.

### Port sudah dipakai

Edit `docker-compose.yml` dan ubah mapping port (angka di kiri, mis. `"8080:8080"` → `"18080:8080"`).
